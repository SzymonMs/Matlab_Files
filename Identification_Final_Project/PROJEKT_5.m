...
FI(3,:)=[0,0,0,0,0];
FI(4,:)=[0,0,0,0,0];
dp = 5; %liczba parametrów- ZAMIANA TU DO KA¯DEJ PÊTLI JU¯ NIE TRZEBA
for i = dp:N
    fi = [-y(i-1),-y(i-2),-y(i-3),u(i-2), u(i-3)];
    FI(i,:) = fi;
    fi = [];
end
...
for i = dp:N
    ...
    ym(i) = -p_N_LS(1)*ym(i-1)-p_N_LS(2)*ym(i-2)-p_N_LS(3)*ym(i-3)+p_N_LS(4)*u(i-2)+p_N_LS(5)*u(i-3);
end
%% Metoda IV
z = [];
Z = [];
...
Z(4,:)=[0,0,0,0,0];
for i = dp:N
    z = [-ym(i-1),-ym(i-2),-ym(i-3),u(i-2), u(i-3)];
    Z(i,:) = z;
    z = [];
end
p_N_IV = inv(Z'*FI)*Z'*y;
...
pred=[];
for i = dp:N
    pred(i) = -p_N_IV(1)*yVal(i-1)-p_N_IV(2)*yVal(i-2)-p_N_IV(3)*yVal(i-3)+p_N_IV(4)*uVal(i-2)+p_N_IV(5)*uVal(i-3);
end
for i = dp:N
    ym2(i) = -p_N_IV(1)*ym2(i-1)-p_N_IV(2)*ym2(i-2)-p_N_IV(3)*ym2(i-3)+p_N_IV(4)*uVal(i-2)+p_N_IV(5)*uVal(i-3);
end
